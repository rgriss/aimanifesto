...
### Fixed
- **ToolsGuideModal - Border Radius Alignment**: Folded corner button now matches parent container radius
  - Added `rounded-tr-lg` to button and triangle elements
  - Matches PageHeader's rounded-lg border radius (0.5rem)
  - Corner now perfectly conforms to parent container's rounded top-right corner
  - Eliminates visual imperfection where button extended beyond container radius
